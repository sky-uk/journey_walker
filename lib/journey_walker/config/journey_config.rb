require_relative 'journey_config_validator'
require_relative 'journey_step_config'
...
class JourneyConfig
  ...
  def initialize(config)
    JourneyConfigValidator.validate(config)
    parse(config)
  end
  ...
  def parse_steps(steps_hash)
    steps_hash.collect { |step| JourneyStepConfig.new(step[:name]) }
  end

  def parse_data_sources(data_sources_hash)
    data_sources_hash.collect { |source| DataSourceConfig.new(source[:name], source[:class_name], source[:methods]) }
  end
